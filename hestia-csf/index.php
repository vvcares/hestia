...
<div class="l-center units" style="margin:1em;text-align: center;">
    ...
    <div id="iframe-container" style="width: 90%; max-width: 1200px; margin: 0 auto;">
        <iframe scrolling='auto' name='myiframe' id='myiframe' frameborder='0' width='100%' 
            onload='resizeIframe(this);' 
            style="min-height: 400px; transition: opacity 0.3s ease-in-out;" 
            sandbox="allow-scripts allow-forms allow-same-origin">
        </iframe>
    </div>
</div>

<div id="csf-loader" style="display: none;">
    <i class="fas fa-spinner fa-spin fa-3x"></i>
    <span>CSF Loading - VVCARES</span>
</div>
...
function loadContent(a,e,t){
    ...
    const f=d.getElementById('csf_submit_form'),
    i=d.getElementById('csf_action_input'),
    iframe=d.getElementById('myiframe'),
    loader=d.getElementById('csf-loader');
    
    if(a){
        // SHOW LOADER: Now uses 'flex' display property for fixed centering
        loader.style.display = 'flex'; 
        // Make the iframe semi-transparent to better highlight the loader
    ...
    // Tab switching logic
    if(t){
        d.querySelectorAll('.csf-nav-button').forEach(b=>b.classList.remove('active'));
        d.querySelectorAll('.csf-sub-nav-content').forEach(c=>c.classList.remove('active'));
        e&&e.classList.add('active');
        d.getElementById(t)&&d.getElementById(t).classList.add('active');
    }
    return!1;
}

function resizeIframe(o){
    const loader = d.getElementById('csf-loader');
    try{
        // Resize logic
        o.contentWindow.document.body.scrollHeight&&(o.style.height=o.contentWindow.document.body.scrollHeight+20+'px');
        // Restore iframe opacity to 1 (fully visible)
        o.style.opacity = '1'; 
    }catch(e){
        // Removed console.warn for minimization, but kept safe try/catch
    } finally {
        // HIDE LOADER
        if (loader) {
...
d.addEventListener('DOMContentLoaded', function() {
    ...
    if(h){
        const p=d.getElementById('btn-'+h),q=d.getElementById('content-'+h);
        if(p&&q){
            b='btn-'+h;c='content-'+h;
    ...
    const ib=d.getElementById(b);ib&&ib.classList.add('active');
    const ic=d.getElementById(c);ic&&ic.classList.add('active');
    loadContent(a,null,null);
    
    d.querySelectorAll('.csf-nav-button').forEach(button=>{button.addEventListener('click',function(){history.pushState(null,null,'#'+this.id.replace('btn-',''))})});
});
</script>
